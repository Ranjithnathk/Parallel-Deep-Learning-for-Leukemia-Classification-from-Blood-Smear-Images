...
def log_results(run_type, num_gpus, epochs, total_time, gpu_type, speedup=None, efficiency=None):
    ...
    log_file = os.path.join("csv_files", f"ddp_amp_gpu_results_{gpu_type}.csv")
...
def train(rank, world_size):
    ...
    if rank == 0:
        ...
        log_results("DDP+AMP", world_size, num_epochs, total_training_time, gpu_type, speedup, efficiency)

        # Save model
        model_path = f"models/{gpu_type}/model_ddp_amp_{gpu_type}_{world_size}gpu.pth"
        os.makedirs(os.path.dirname(model_path), exist_ok=True)
        torch.save(model.module.state_dict(), model_path)
        print(f"Model saved to {model_path}")

        # Save training metrics
        metrics_path = f"training_metrics/{gpu_type}_metrics_ddp_amp_{world_size}gpu.npz"
